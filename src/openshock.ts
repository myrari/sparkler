...
import { Sink, sessions } from "./api.js";

// const OPENSHOCK_TOKEN = process.env.OPENSHOCK_TOKEN;

interface Control {
    id: string; //control id?
    intensity: number; //int32 : 0 - 100
    duration: number; //int32 : 300 - 65535 (time in millis?)
    type: string; //types: "Stop", "Shock", "Sound", "Vibrate"
    exclusive: boolean; //stop other active controls?
}
...
interface OpenShockSink extends Sink {
    host: string;
    userAgent: string;
...
function newOpenShockSink(token: string): OpenShockSink {
    return {
...
export function addOpenShockRoutes(app: Express) {
    ...
    app.post("/openshock/auth", (req, res) => {
        ...
        const shockers: string[] = req.body.shockers;
